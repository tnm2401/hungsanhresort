          ...
          <div class="box box-primary">
            <div class="box-body">
              <div class="form-group">
                <label>Tên dịch vụ ({{ session('locale') }})</label>
              ...
              <div class="form-group">
                ...
                <textarea class="form-control" name="translation[descriptions]" id="descriptions"
                    ...
                    title="Nhập mô tả ngắn sản phẩm">{!!$svcategory->translations->descriptions !!}</textarea>
            </div>

            <div class="form-group">
                <label>Nội dung ({{session('locale')}})</label>
                <textarea class="form-control" name="translation[content]" id="content" rows="3">{!!$svcategory->translations->content !!}</textarea>
              ...
              <div class="box-body">
                <div class="form-group">
                  ...
                  <div class="description-seo" id="description-seo">{{ $svcategory->translations->description }}</div>
                  <label>URL danh mục</label>
                  <input type="text" type="text" name="translation[slug]" id="slug" value="@if(isset($svcategory->translations->slug)){{ old('slug', $svcategory->translations->slug) }}@else{{ old('slug') }}@endif" class="form-control" data-toggle="tooltip" data-placement="top" title="Nhập URL danh mục">
                </div>
                <div class="form-group">
                  <label>Tiêu đề ({{ session('locale') }})</label>
                  <input type="text" name="translation[title]" id="title" value="@if(isset($svcategory->translations->title)){{ old('title', $svcategory->translations->title) }}@else{{ old('title') }}@endif" class="form-control" data-toggle="tooltip" data-placement="top" title="Nhập tiêu đề danh mục">
                </div>
                <div class="form-group">
                  <label>Từ khóa ({{ session('locale') }})</label>
                  <textarea class="form-control" name="translation[keywords]" id="keywords" value="@if(isset($svcategory->translations->keywords)){{ old('keywords', $svcategory->translations->keywords) }}@else{{ old('keywords') }}@endif" rows="3" data-toggle="tooltip" data-placement="top" title="Nhập từ khoá cho danh mục">@if(isset($svcategory->translations->keywords)){{ old('keywords', $svcategory->translations->keywords) }}@else{{ old('keywords') }}@endif</textarea>
                </div>
                <div class="form-group">
                  <label>Mô tả ({{ session('locale') }})</label>
                  <textarea class="form-control" name="translation[description]" id="description" value="@if(isset($svcategory->translations->description)){{ old('description', $svcategory->translations->description) }}@else{{ old('description') }}@endif" rows="7" data-toggle="tooltip" data-placement="top" title="Nhập mô tả ngắn danh mục">@if(isset($svcategory->translations->description)){{ old('description', $svcategory->translations->description) }}@else{{ old('description') }}@endif</textarea>
                </div>
