...
@section('content')
    ...
    <div class="content-wrapper" id="pjax-container">
        ...
        <section class="content">
            ...
            @endif
            <form id="stringLengthForm" method="POST" action="{{ route('backend.post.store') }}"
                                    ...
                                    <div class="tab-pane {{ $loop->first ? 'in active' : '' }} tab-content-en"
                                        id="tab_{{ $lang->locale }}">
                                            <div class="form-group">
                                                <label>Tên bài viết ({{ $lang->locale }})</label>
                                                <input type="text" name="translation[{{ $key }}][name]"
                                                    id="name_{{ $lang->locale }}" onkeyup="AutoSlug();" data-length=120
                                                    value="{{ old('translation[' . $key . '][name]') }}"
                                                    class="form-control" data-toggle="tooltip" data-placement="top"
                                                    title="Nhập tên danh mục">
                                            </div>
                                            @if ($loop->first)
                                                <div class="form-group">
                                                    <label>Tag ({{ $lang->locale }})</label> <small>*có thể chọn nhiều
                                                        tags</small>
                                                    <select class="form-control tag select2" name=tags[]
                                                        multiple="multiple">
                                                        @foreach ($tags as $tag)
                                                            <option value="{{ $tag->id }}">
                                            ...
                                            @endif
                                            <div class="form-group">
                                                <label>Mô tả bài viết ({{ $lang->locale }})</label>
                                                <textarea class="form-control" name="translation[{{ $key }}][descriptions]"
                                                    id="descriptions_{{ $lang->locale }}" onchange="getDescription()"
                                                    value="{{ old('translation[' . $key . '][descriptions]') }}" rows="3"
                                                    data-toggle="tooltip" data-placement="top"
                                                    title="Nhập mô tả bài viết {{ $lang->locale }}">{{ old('descriptions_vi') }}</textarea>
                                            </div>
                                            <div class="form-group">
                                                <label>Nội dung ({{ $lang->locale }})</label>
                                                <textarea class="form-control" name="translation[{{ $key }}][content]" id="content_{{ $lang->locale }}"
                                                    rows="3">{!! old('content_vi') !!}</textarea>
                                            </div>
